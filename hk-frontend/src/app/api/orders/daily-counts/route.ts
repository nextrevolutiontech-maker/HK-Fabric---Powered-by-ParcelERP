...
export async function GET(request: Request) {
  try {
    ...
    const dailyCounts = await OrderService.getDailyParcelCounts({
      startDateStr,
      endDateStr,
      orderType
    });
    ...
    console.error('Error fetching daily parcel counts:', error);
    return NextResponse.json({ error: 'Failed to fetch daily parcel counts' }, { status: 500 });
  }
}
